...
router.get('/', function (req, res, next) {
    ...
    const today = moment().format('YYYYMMDD-') +  moment().add(1, 'days').format('YYYYMMDD');
    let resultPath = '';
    let outputFiles = [];
    ...
    let getAllRoutes = function (callback) {
        allRoutes = routes.find({userID: userID})
            .populate('serviceID')
    ...
    let createFile = function (content, name, callback) {
        fileHelper.writeFile(userID, userTime, name, content, function (err, resultat) {
            if (err) {
                return callback(JSON.stringify(err), null);
            }
            callback(null, true);
    ...
    let createAgenciesFile = function (callback) {
        agencies.findOne({userID: userID}, function (err, agn) {
            ...
            let content = 'agency_id,agency_name,agency_url,agency_timezone,agency_lang,agency_phone,agency_fare_url' + "\n";
            content += agn.agency_id + ',' + agn.agency_name + ',' + agn.agency_url + ',' + agn.agency_timezone + ',' + agn.agency_lang + ',' + agn.agency_phone + ',' + agn.agency_fare_url + "\n";
    ...
    let createFareRulesFile = function (callback) {
        let content = 'fare_id,route_id,origin_id,destination_id,contains_id' + "\n";
    ...
    let createFareAttributesFile = function (callback) {
        let content = 'fare_id,price,currency_type,payment_method,transfers,transfer_duration' + "\n";
    ...
    let createFeedInfoFile = function (callback) {
        let content = 'feed_publisher_name,feed_publisher_url,feed_lang,feed_version' + "\n"
    ...
    let convert = function(dateString) {
        ...
        let month = dateString.substr(4, 2);//20120101
    ...
    let createCalendarFile = function (callback) {
        services.find({userID: userID}, function (err, rez) {
            ...
            rez.forEach(function (agn) {
                content += agn.serviceName + ',' + (+agn.Monday) + ',' + (+agn.Tuesday) + ',' + (+agn.Wednesday) + ',' + (+agn.Thursday) + ',' + (+agn.Fryday) + ',' + (+agn.Saturday) + ',' + (+agn.Sunday) + ',' + agn.startDate + ',' + agn.endDate + "\n";
                let newCalendarRecord = new gtfsCalendarModel(
                    {
                        userID: userID,
                        ...
                        sunday: agn.Sunday,
                        start_date: convert(agn.startDate),
                        end_date: convert(agn.endDate)
                    }
                    );
    ...
    let createStopsFile = function (callback) {
        stops.find({userID: userID}, function (err, rez) {
            ...
            let content = 'stop_id,stop_code,stop_name,stop_desc,stop_lat,stop_lon,zone_id,stop_url,location_type,parent_station' + "\n";
            rez.forEach(function (agn) {
                content += agn.stop_id + ',' + agn.stop_code + ',' + agn.stop_name + ',' + agn.stop_desc + ',' + agn.stop_lat + ',' + agn.stop_lon + ',' + agn.zone_id + ',' + agn.stop_url + ',' + agn.location_type + ',' + agn.parent_station + "\n";
            });
    ...
    let createRoutesFile = function (callback) {
        routes.find({userID: userID}, function (err, availableRoutes) {
            ...
            availableRoutes.forEach(function (anotherRoute) {
                ...
                aRoutes[availableRoutesIndex]['shape_index_ID'] = 'Route_' + anotherRoute.routeID + '_' + anotherRoute._id;
                availableRoutesFrequencies[availableRoutesIndex] = [];
                anotherRoute.frequency.forEach(function (felement, findex, farray) {
                    availableRoutesFrequencies[availableRoutesIndex][findex] = {fr: null, time: null};
                    availableRoutesFrequencies[availableRoutesIndex][findex].fr = parseInt(felement.fr, 10) * minToSeconds;
                    availableRoutesFrequencies[availableRoutesIndex][findex].time = +new Date(moment(startDayPattern + felement.fromHr + ':' + anotherRoute.startRouteTime.fromMin, siteConfig.fullDatePattern).format())
                });
                ...
                content += anotherRoute.routeID + ',' + agencyId + ',' + anotherRoute.routeName + ',' + anotherRoute.routeName + ' ' + anotherRoute.routeDesc + ',' + anotherRoute.routeDesc + ',' + '3' + ',' + '' + ',' + getRandomColor() + ',' + "\n";
    ...
    let createShapesFile = function (callback) {
        fileHelper.createShapesSequence(userID, userTime, allRoutes, function (err, data) {
            if (err) {
                console.log(err);
                return callback(err, null);
            } else {
    ...
    let createStopTimesFile = function (callback) {
        fileHelper.createStopTimeFile(userID, userTime, allRoutes, function (err, data) {
            if (err) {
                console.log(err);
                return callback(err, null);
            } else {
    ...
    async.series([
        ...
        function (err, results) {
            ...
            let file = './helpers/downloads/' + userID + '/' + userTime + '/download.zip';
